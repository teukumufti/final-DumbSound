    ...
    try {
      ...
      if (response.status == "success") {
        const alert = (
          <Alert
            variant="success"
            className="py-1 text-center m-auto w-20 mt-5"
          >
            Success add Music
          </Alert>
        );
      ...
      <div className="admin-container text-white">
        ...
        >
          ...
          <div className="row gap-2">
            <input
              type="text"
              className="p-2 bg-grey col-8 border-none borra-2"
              placeholder="Title"
              value={title}
              name="title"
            ...
            <div class=" col fs-7 thumbnail">
              <input
                ...
                hidden
              />
              <label class="input-group-text text-grey" for="thumbnail">
                Attach Thumbnail<i class="bx bx-paperclip ms-2"></i>
              </label>
            </div>

            <input
              type="text"
              className="form-control p-2 bg-grey my-3"
              placeholder="Year"
              value={year}
              name="year"
              onChange={handleChange}
            ...
            <select
              class="form-select bg-grey mb-3 text-grey1"
              type="text"
              onChange={handleChange}
              value={idArtis}
              name="idArtis"
            ...
            >
              ...
              {/* maping artis option */}
              {artiss?.map((artis) => (
                <option name="idArtis" value={artis.id}>
                  {artis.name}
                </option>
              ))}
            </select>
            ...
            <div className="attache">
              ...
              <label
                for="attache"
                class="w-20 mb-3 input-group-text text-grey"
                accept="audio/*"
              >
                Attache
              </label>
          ...
          <div className="d-flex justify-content-center my-3">
            <button
              type="submit"
              className="bg-yellow w-30 text-white fw-500 py-2 borra-3"
            >
              Submit
            </button>
          </div>
